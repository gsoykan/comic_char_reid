...
from src.utils.pytorch_utils import LambdaLayer
...
class PMLIdNetFineTunedSSLBackboneFaceBodyLitModule(LightningModule):
    def __init__(
            ...
            ssl_backbone_face: SSLBackbone = SSLBackbone.SIM_CLR_DEEPER_LAST,
            # body params
            backbone_latent_dim_body: int = 512,
            ssl_backbone_body: SSLBackbone = SSLBackbone.SIM_CLR_DEEPER_LAST,
    ...
    def load_backbone(self, ssl_backbone: SSLBackbone, ssl_ckpt) -> nn.Module:
        if ssl_backbone in [SSLBackbone.SIM_CLR,
            ...
            raise Exception(f'unhandled backbone for id net fine-tuning: {self.ssl_backbone_face}')
...
def check_embedding_generation():
    model_checkpoint = '/home/gsoykan20/Desktop/self_development/char_reid_repo/data/checkpoints/face_body_aligned_ssl.ckpt'
    ssl_model_ckpt = '/home/gsoykan20/Desktop/self_development/char_reid_repo/data/checkpoints/semi_supervised_id_net.ckpt'
    trained_model = PMLIdNetFineTunedSSLBackboneFaceBodyLitModule.checkpoint_to_eval(model_checkpoint,
                                                                                     ssl_ckpt_face=ssl_model_ckpt,
                                                                                     ssl_ckpt_body=ssl_model_ckpt)
    face_img_paths = [
